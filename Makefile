...
KIND_CLUSTER ?= mcp-operator-test-e2e
...
.PHONY: test-e2e
test-e2e: setup-test-e2e manifests generate build-installer-all fmt vet ## Run the e2e tests. Expected an isolated environment using Kind.
...
license-check: ## Check if all files have license headers
	...
	if [ $$missing -eq 1 ]; then \
		echo "Some files are missing license headers. Run 'make license' to add them."; \
		exit 1; \
	else \
